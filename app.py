...
def download_sec_filing(ticker: str, form_type: str = "10-K", limit: int = 1):
    """
    Download the latest SEC filing (10-K or 10-Q) for the given ticker.
    Returns a list of file paths.
    """
    try:
        dl = Downloader()
        dl.get(form_type, ticker, amount=limit)
        path = f"sec-edgar-filings/{ticker}/{form_type.lower()}/"
        files = []
        ...
        st.error(f"SEC filing download failed for {ticker}: {e}")
        return []
    ...
    with col_left:
        ...
        with col2:
            ...
            with col2:
                ...
                st.markdown("📥 **Latest SEC Filing (Downloaded)**")

                filings = download_sec_filing(sym, "10-K")

                if filings:
                    latest = filings[0]
                    st.markdown(f"- [📄 View Latest 10-K Filing for `{sym}`]({latest})")
                else:
                    st.info("No downloaded 10-K filings found.")
